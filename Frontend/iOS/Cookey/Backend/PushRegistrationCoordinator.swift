...
import Observation
    ...
    final class PushRegistrationCoordinator {
        ...
        func handleRegisteredDeviceToken(_ token: Data) async {
            guard case let .waitingForToken(serverURL, deviceID) = state else {
                return
            }
    ...
    final class PushRegistrationCoordinator {
        ...
        func beginRegistration(serverURL _: URL, deviceID _: String) async {}

        func handleRegisteredDeviceToken(_: Data) async {}

        func handleRegistrationFailure(_: Error) {}

        func handleNotificationUserInfo(_: [AnyHashable: Any]) {}

        func attach(to _: SessionUploadModel) async {}
    }
#endif
