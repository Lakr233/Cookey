...
public actor RequestStorage {
    ...
    public func cleanupExpired() -> [RequestID] {
        let now = Date()
        let expired = requests.keys.filter { requests[$0]!.expiresAt < now }
    ...
    public func waitForMessage(rid: RequestID, timeoutSeconds: Int) async -> WebSocketMessage? {
        ...
        if case let .error(payload) = message, payload.code == "timeout" || payload.code == "cancelled" {
            return nil
        }
    ...
    public func cancelWait(rid: RequestID) {
        notifyWebSocketListeners(
            rid: rid,
            message: .error(ErrorPayload(code: "cancelled", message: "Waiting cancelled"))
        )
    }
