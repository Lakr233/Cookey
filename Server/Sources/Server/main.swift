...
enum CookeyServer {
    ...
    static func parseArguments() -> ServerConfig {
        ...
        if let envHost = env["COOKEY_HOST"] { host = envHost }
        if let envPort = env["COOKEY_PORT"].flatMap(Int.init) { port = envPort }
        if let envPublicURL = env["COOKEY_PUBLIC_URL"] { publicURL = envPublicURL }
...
func runServer() async throws {
    ...
    let apnsClient = config.apnsConfiguration.map { APNSClient(configuration: $0) }
